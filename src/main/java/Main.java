...
public class Main {
    ...
    // Bash tracks exactly two pointers into job history: the "current" job (+)
    // and the "previous" job (-). There is no deeper backfill history beyond
    // these two slots.
    private static Integer currentJob = null;
    private static Integer previousJob = null;
    ...
    private static void runJobsBuiltin() {
        synchronized (Main.class) {
            TreeMap<Integer, Job> sorted = new TreeMap<>(jobs);
            for (Map.Entry<Integer, Job> entry : sorted.entrySet()) {
                int jobNum = entry.getKey();
                char sign = signForLocked(jobNum);
    ...
    // Must be called while holding the Main.class lock.
    private static char signForLocked(int jobNum) {
        if (currentJob != null && currentJob == jobNum) {
            return '+';
        }
        if (previousJob != null && previousJob == jobNum) {
            return '-';
        }
        return ' ';
    }
    ...
    private static void runPipelineBackground(List<String> stageStrings, String commandLine) {
        ...
        try {
            ...
            synchronized (Main.class) {
                jobs.put(jobNum, job);
                // New background job becomes current; old current becomes previous.
            ...
            Thread monitor = new Thread(() -> {
                ...
                String notification;
                synchronized (Main.class) {
                    // Capture this job's marker based on state BEFORE we mutate
                    ...
                    // Bash does NOT backfill from deeper history. Only the slot
                    // that the finishing job occupied gets cleared/promoted.
                    if (currentJob != null && currentJob == jobNum) {
                        currentJob = previousJob;
                        previousJob = null;
                    } else if (previousJob != null && previousJob == jobNum) {
                        previousJob = null;
                    }
                }
